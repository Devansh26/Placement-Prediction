	...
	if(isset($_POST['email']) && $_POST['email'] != "")
	{
	 ...
	 //if user already did, redirect to index.php with error
		if(!(mysqli_num_rows($userCheck) < 1)){
			$user_msg = 'Sorry, but '.$email.', has already attempted the quiz, '.$quzz_name.'!';
			header('location: index.php?user_msg='.$user_msg.'');
			exit();
		}else{
	 //else inserting few columns into the table
		mysqli_query($conn,"INSERT INTO quiz_takers (email, username, percentage, date_time, quiz_id, duration) 
					 VALUES ('$email','1', '0', now(), '$final_quiz_ID', '0')")or die(mysql_error());
		}
	}else{
 ...
 //Getting the questions from DB here
	$m_questions_from_DB = mysqli_query($conn,"SELECT * FROM questions WHERE quiz_id='$final_quiz_ID'
	 ...
	 //adding html for hidden values to be sent to result.php
		$m_output .= '<input type="hidden" name="rollno" value="'.$email.'">
