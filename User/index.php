    ...
    <div class="main-content" id="home">
        ...
        <div class="banner-content-w3pvt">
            <div class="banner-w3layouts-info text-center">
                ...
                <form class="banner-form" method="post" action="index.php" enctype="multipart/form-data">        
                <?php  
                    $sql1="select * from files where email='$user' ";
                    $retval=mysqli_query($conn,$sql1);
                    //$r=mysqli_fetch_assoc($sql1);
                    if($retval){
                        $numberAffectedRow = mysqli_num_rows($retval);
                        //$result=mysqli_fetch_array($retval);
                        //echo $numberAffectedRow;
                        if($numberAffectedRow>0){ ?>      
                            <a href="view.php" target="_blank">View Resume</a><br>
                            <input type="file" class="form-control" name="myfile">
                            <button type="submit" name="save" class="btn btn-default">Upload New</button>
                        <?php } else { ?>
                            <input type="file" class="form-control" name="myfile">
                            <button type="submit" name="save" class="btn btn-default">Upload Resume</button>                        
                        <?php } ?>    
                    <?php } else { ?>
                        <script type="text/javascript">
                        window.location.href='index.php';
                        alert("This user has not uploaded Resume, please upload it ");
                        </script>                             
